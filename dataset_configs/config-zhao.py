...
def get_config():
    ...
    config.input_name = "gnfw_profiles/gnfw_beta_priorlarge_pois100/data.0.hdf5"
    config.output_root = "/mnt/ceph/users/tnguyen/jeans_gnn/datasets/processed_datasets"
    config.output_name = "gnfw_profiles/gnfw_beta_priorlarge_pois100/data.0.hdf5"
    config.seed = 42
    ...
    config.max_radius_kpc = 20   # maximum radius in kpc
    config.norm_rstar = False

    config.condition = config_dict.ConfigDict()
    config.condition.fit_lp = True
    ...
    config.lp.fn = 'zhao'
    config.lp.p0 = [0., 0., 2, 5, 0.]
    config.lp.bounds = ([-10, -2, 0.5, 2, 0], [10, 2, 5, 10, 5])
